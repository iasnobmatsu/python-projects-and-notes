...
class NewPostForm(FlaskForm):
    ...
    ismarkdown=BooleanField("MarkDown")
    article=TextAreaField("Content", validators=[DataRequired()])
    submit=SubmitField("Post")

class SortForm(FlaskForm):
    order= SelectField('view by order', choices=[("newest"),('oldest')])
...
class ResetEmailForm(FlaskForm):
    ...
    def validate_email(self, email):
        email=User.query.filter_by(email=email.data).first()
        if email is None:
            raise ValidationError("No account exists with the email you entered.")
